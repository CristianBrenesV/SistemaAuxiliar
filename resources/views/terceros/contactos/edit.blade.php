...
<div class="container-fluid">
    <h1 class="mt-4">
        Editar Contacto para: <span class="text-primary">{{ $tercero->Nombre }}</span>
    </h1>
    ...
    <div class="d-flex mb-3 gap-2">
        ...
        <button type="submit" class="btn btn-dark" form="formEditarContacto">
            ...
            <div class="card shadow-sm border-0" style="background-color: #d9d9d9;">
                <div class="card-body">
                    <form id="formEditarContacto" 
                          action="{{ route('terceros.contactos.actualizar', [$tercero->IdTercero, $contacto->IdContacto]) }}" 
                          method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" 
                                   ...
                                   value="{{ old('nombre_contacto', $contacto->NombreContacto) }}"
                                   maxlength="100"
                                   required>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('tipo_contacto') is-invalid @enderror" 
                                ...
                                <option value="Principal" {{ old('tipo_contacto', $contacto->TipoContacto) == 'Principal' ? 'selected' : '' }}>Principal</option>
                                <option value="Facturación" {{ old('tipo_contacto', $contacto->TipoContacto) == 'Facturación' ? 'selected' : '' }}>Facturación</option>
                                <option value="Cobros" {{ old('tipo_contacto', $contacto->TipoContacto) == 'Cobros' ? 'selected' : '' }}>Cobros</option>
                                <option value="Soporte" {{ old('tipo_contacto', $contacto->TipoContacto) == 'Soporte' ? 'selected' : '' }}>Soporte</option>
                                <option value="Otro" {{ old('tipo_contacto', $contacto->TipoContacto) == 'Otro' ? 'selected' : '' }}>Otro</option>
                            </select>
                            @error('tipo_contacto')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('estado') is-invalid @enderror" 
                                ...
                                <option value="1" {{ old('estado', $contacto->Estado) == 1 ? 'selected' : '' }}>Activo</option>
                                <option value="0" {{ old('estado', $contacto->Estado) == 0 ? 'selected' : '' }}>Inactivo</option>
                            </select>
                            @error('estado')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
